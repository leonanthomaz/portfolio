...
const Loading = ({ isVisible }: { isVisible: boolean }) => {
  ...
  // Atualiza o progresso de 0 a 100%
  useEffect(() => {
    const totalSteps = pythonCode.length;
    setProgress(Math.min(Math.round(((lineIndex + (currentLine.length / (pythonCode[lineIndex]?.length || 1))) / totalSteps) * 100), 100));
  }, [lineIndex, currentLine, pythonCode]);
  ...
  return (
    <Box sx={{
      ...
      {/* Logo Python */}
      <Box sx={{ animation: `${pulse} 2s infinite`, mb: 3 }}>
      ...
      {/* Console */}
      <CodeSnippet sx={{ maxWidth: isMobile ? '90%' : '600px' }}>
        {codeLines.map((line, index) => <CodeLine key={index}>{line}</CodeLine>)}
        {currentLine && (
          <CodeLine>
            {currentLine}
